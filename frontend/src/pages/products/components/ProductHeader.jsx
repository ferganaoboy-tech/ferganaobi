...
const ProductHeader = ({ totalProductsCount, openCreateModal, openAiParser, handleExportExcel }) => {
  return (
    <div className="flex items-center justify-between mb-[24px] shrink-0 gap-3">
      ...
      <div className="flex items-center gap-2 sm:gap-3">
        <button 
          ...
          className="w-10 h-10 sm:w-auto sm:h-9 sm:px-4 bg-emerald-50 text-emerald-600 rounded-full sm:rounded-md text-13 font-[500] hover:bg-emerald-100 active:scale-[0.98] transition-all flex items-center justify-center gap-2 shrink-0 shadow-sm border border-emerald-200"
          title="Excel'ga saqlash"
        >
          <FileSpreadsheet className="w-5 h-5 sm:w-4 sm:h-4" strokeWidth={1.5} />
